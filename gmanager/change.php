...
switch ($_GET['go']) {
    ...
    case 'scan':
        if (!isset($_POST['url']) || $_POST['url'] == 'http://') {
            ...
            if ($url = $Gmanager->getData($_POST['url'], $_POST['headers'], $only_headers, $_POST['post'])) {
                ...
                echo '<div class="code">IP: <span style="font-weight: normal;">' . implode(', ', gethostbynamel(parse_url($_POST['url'], PHP_URL_HOST))) . '</span><br/>' . Language::get('size') . ': <span style="font-weight: normal;">' . $Gmanager->formatSize(strlen($url)) . '</span><br/></div>' . $Gmanager->code($url, 0, true);
            } else {
                echo Errors::message(Language::get('not_connect'), Errors::MESSAGE_EMAIL);
            }
        }
    ...
    case 'sqlite':
        ...
        if (isset($_POST['db'])) {
            ...
            echo '<div class="input"><form action="change.php?go=sqlite&amp;c=' . Config::$rCurrent . '" method="post" id="post"><div>' . Language::get('sql_db') . '<br/><input type="text" name="db" value="' . Config::$hCurrent . '"/><br/>' . Language::get('charset') . '<br/><input type="text" name="charset" value="utf8"/><br/>' . Language::get('sql_query') . '<br/><textarea id="sql" name="sql" rows="4" cols="48">' . htmlspecialchars($_POST['sql'], ENT_NOQUOTES) . '</textarea><br/><input type="submit" value="' . Language::get('sql') . '"/></div></form></div>';
        }
        break;
    ...
    case 'sql_tables_sqlite':
        if (!(isset($_POST['tables']) && $Gmanager->is_file($_POST['tables'])) && !(isset($_FILES['f_tables']) && !$_FILES['f_tables']['error'])) {
            echo '<div class="input"><form action="change.php?go=sql_tables_sqlite&amp;c=' . Config::$rCurrent . '" method="post" enctype="multipart/form-data"><div>' . Language::get('sql_db') . '<br/><input type="text" name="db"/><br/>' . Language::get('tables_file') . '<br/><input type="text" name="tables" value="' . Config::$hCurrent . '" style="width:40%"/><input type="file" name="f_tables" style="width:40%"/><br/><input type="submit" value="' . Language::get('tables') . '"/></div></form></div>';
        } else {
            Config::$sqlDriver = 'sqlite';
            echo $Gmanager->sqlQuery('', '', '', $_POST['db'], '', !$_FILES['f_tables']['error'] ? file_get_contents($_FILES['f_tables']['tmp_name']) : $Gmanager->file_get_contents($_POST['tables']));
        }
        break;
