...
class SQL_PostgreSQL implements SQL_Interface
{
    ...
    public function backup($host = null, $name = null, $pass = null, $db = '', $charset = null, $tables = [])
    {
        ...
        if ($tables) {
            ...
            if ($tables['data']) {
                foreach ($tables['data'] as $f) {
                    ...
                    if ($q) {
                        ...
                        $false .= \pg_last_error($this->_resource)."\n";
                    }
                }
            }
            ...
            if ($false) {
                return Helper_View::message(Language::get('sql_backup_false').'<pre>'.\trim($false).'</pre>', Helper_View::MESSAGE_ERROR);
            }
    ...
    public function query($host = null, $name = null, $pass = null, $db = '', $charset = null, $data = '')
    {
        ...
        foreach (SQL::parser($data) as $q) {
            ...
            if (!$r) {
                return Helper_View::message(Language::get('sql_query_false'), Helper_View::MESSAGE_ERROR_EMAIL).'<div><code>'.\pg_last_error($this->_resource).'</code></div>';
            }
