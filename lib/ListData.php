...
//TODO: create method getHeader (create header for ListData)
class ListData
{
    /**
     * @var int
    ...
    private static function _getListSearchArray($where = '', $pattern = '', $inText = false, $limit = 8388608, $archive = false, $t = '')
    {
        ...
        foreach ($obj->iterator($where) as $f) {
            ...
            if ($inText) {
                if ($stat['size'] > $limit || ($arch && !$archive) || ($arch && $archive && Archive::FORMAT_GZ != $type)) {
                    continue;
                }

                $in = \preg_match_all($pattern, (Archive::FORMAT_GZ == $type) ? $obj->getGzContent($where.$f) : $obj->file_get_contents($where.$f), $match);
                unset($match);
                if ($in) {
                    $in = ' ('.$in.')';
                } else {
    ...
    public static function getListSearchData($where = '', $search = '', $inText = false, $caseLess = false, $regexp = false, $limit = 8388608, $archive = false)
    {
        ...
        if (false === \preg_match_all($pattern, '', $match)) {
