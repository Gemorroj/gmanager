...
if (isset($_GET['get']) && Gmanager::getInstance()->is_file($_GET['get'])) {
    if (isset($_GET['f'])) {
        ...
        if ($archive && Archive::FORMAT_GZ != $archive) {
            $obj = new Archive();
...
if ('link' === Registry::get('currentType')) {
    ...
    if ($realpath && 'dir' === Registry::get('currentType')) {
        $realpath .= '/';
    }
}
...
switch ($_GET['go']) {
    case 1:
        ...
        if (isset($_POST['fname'])) {
            ...
            if (!isset($_POST['name']) || !isset($_POST['chmod'])) {
                ...
                } elseif (Archive::FORMAT_GZ == $archive) {
            ...
            if (isset($_POST['dir'])) {
                ...
                if ($archive && Archive::FORMAT_GZ != $archive && Archive::FORMAT_RAR != $archive) {
                    $obj = new Archive();
                    echo $obj->setFormat($archive)->setFile($_POST['add_archive'])->factory()->addFile($_POST['check'], $_POST['dir']);
                }
            } else {
                echo '<div class="input"><form action="?gmanager_action=change&amp;go=1&amp;c='.Registry::get('rCurrent').'" method="post"><div>'.Language::get('add_archive_dir').'<br/><input type="text" name="dir" value="./"/><br/><input name="add_archive" type="hidden" value="'.$_POST['add_archive'].'"/>';
                for ($i = 0; $i < $x; ++$i) {
                    echo '<input name="check[]" type="hidden" value="'.$_POST['check'][$i].'"/>';
                }
    ...
    case 'del':
        if ('dir' == Registry::get('currentType')) {
    ...
    case 'create_file':
        if (!isset($_POST['name'])) {
            ...
            if (Gmanager::getInstance()->file_exists(Registry::get('current').$_POST['name']) && !isset($_POST['a'])) {
                ...
                echo '<div class="border">'.Language::get('file').' <strong><a href="?gmanager_action=edit&amp;c='.Registry::get('rCurrent').\rawurlencode($_POST['name']).'">'.$realpath.'</a></strong> ('.$_POST['chmod'].')<br/></div>'.Gmanager::getInstance()->createFile(Registry::get('current').$_POST['name'], \rawurldecode($_POST['ptn']), $_POST['chmod']);
            }
        }
        break;
    ...
    case 'rename':
        if (isset($_POST['name']) && '' != $_POST['name']) {
            ...
            if (isset($_GET['f']) && $archive && Archive::FORMAT_GZ != $archive && Archive::FORMAT_RAR != $archive) {
                $obj = new Archive();
                ...
                echo Gmanager::getInstance()->frename(Registry::get('current'), $_POST['name'], $_POST['chmod'] ?? null, isset($_POST['del']), $_POST['name'], isset($_POST['overwrite']));
    ...
    case 'upload':
        if ((((!isset($_POST['url']) || 'http://' == $_POST['url'] || '' == $_POST['url']) && (!isset($_FILES['f']) || $_FILES['f']['error'][0])) && !isset($_POST['f'])) || !isset($_POST['name']) || !isset($_POST['chmod'])) {
            ...
            if (!$_FILES['f']['error'][0]) {
                ...
                echo Gmanager::getInstance()->uploadUrl($_POST['url'], $_POST['name'], $_POST['chmod'], $_POST['headers'], $_POST['set_time_limit'] ?? false, isset($_POST['ignore_user_abort']));
            }
        }
        break;
    ...
    case 'mod':
        ...
        echo '<div class="red"><ul><li><a href="?gmanager_action=change&amp;go=search&amp;c='.Registry::get('rCurrent').'">'.Language::get('search').'</a></li><li><a href="?gmanager_action=change&amp;go=eval&amp;c='.Registry::get('rCurrent').'">'.Language::get('eval').'</a></li><li><a href="?gmanager_action=change&amp;go=cmd&amp;c='.Registry::get('rCurrent').'">'.Language::get('cmd').'</a></li><li>SQL<ul><li><a href="?gmanager_action=change&amp;go=mysql&amp;c='.Registry::get('rCurrent').'">MySQL</a></li><li><a href="?gmanager_action=change&amp;go=postgresql&amp;c='.Registry::get('rCurrent').'">PostgreSQL</a></li><li><a href="?gmanager_action=change&amp;go=sqlite&amp;c='.Registry::get('rCurrent').'">SQLite</a></li></ul></li><li><a href="?gmanager_action=change&amp;go=sql_tables&amp;c='.Registry::get('rCurrent').'">'.Language::get('tables').'</a></li><li><a href="?gmanager_action=change&amp;go=sql_installer&amp;c='.Registry::get('rCurrent').'">'.Language::get('create_sql_installer').'</a></li><li><a href="?gmanager_action=change&amp;go=scan&amp;c='.Registry::get('rCurrent').'">'.Language::get('scan').'</a></li><li><a href="?gmanager_action=change&amp;go=send_mail&amp;c='.Registry::get('rCurrent').'">'.Language::get('send_mail').'</a></li><li><a href="?gmanager_action=change&amp;go=phpinfo&amp;c='.Registry::get('rCurrent').'">'.Language::get('phpinfo').'</a> ('.\PHP_VERSION.')</li><li><a target="_blank" href="https://github.com/Gemorroj/gmanager/releases">'.Language::get('new_version').'</a></li></ul>'.($php_user['name'] ? '<span style="color:#000;">&#187;</span> '.Language::get('php_user').\htmlspecialchars($php_user['name'], \ENT_NOQUOTES).'<br/>' : '').'<span style="color:#000;">&#187;</span> '.\htmlspecialchars($_SERVER['SERVER_SOFTWARE'], \ENT_NOQUOTES).'<br/><span style="color:#000;">&#187;</span> '.\htmlspecialchars(Gmanager::getInstance()->getUname(), \ENT_NOQUOTES).'<br/><span style="color:#000;">&#187;</span> '.Language::get('disk_total_space').' '.Helper_View::formatSize(@\disk_total_space($_SERVER['DOCUMENT_ROOT'])).'; '.Language::get('disk_free_space').' '.Helper_View::formatSize(@\disk_free_space($_SERVER['DOCUMENT_ROOT'])).'<br/><span style="color:#000;">&#187;</span> '.\date('d.m.Y / H').'<span style="text-decoration:blink;">:</span>'.\date('i').'<br/></div>';
        break;

    case 'scan':
        if (!isset($_POST['url']) || 'http://' == $_POST['url']) {
            echo '<div class="input"><form action="?gmanager_action=change&amp;go=scan&amp;c='.Registry::get('rCurrent').'" method="post"><div>'.Language::get('url').'<br/><input type="text" name="url" value="http://"/><br/>'.Language::get('headers').'<br/><textarea class="lines" rows="3" cols="32" name="headers">User-Agent: '.(isset($_SERVER['HTTP_USER_AGENT']) ? \htmlspecialchars($_SERVER['HTTP_USER_AGENT'], \ENT_NOQUOTES) : '')."\n".'Cookie: '."\n".'Referer: '."\n".'Accept: '.(isset($_SERVER['HTTP_ACCEPT']) ? \htmlspecialchars($_SERVER['HTTP_ACCEPT'], \ENT_NOQUOTES) : '')."\n".'Accept-Charset: '.(isset($_SERVER['HTTP_ACCEPT_CHARSET']) ? \htmlspecialchars($_SERVER['HTTP_ACCEPT_CHARSET'], \ENT_NOQUOTES) : '')."\n".'Accept-Language: '.(isset($_SERVER['HTTP_ACCEPT_LANGUAGE']) ? \htmlspecialchars($_SERVER['HTTP_ACCEPT_LANGUAGE'], \ENT_NOQUOTES) : '')."\n".'Connection: Close'."\n".'</textarea><br/>POST<br/><input type="text" name="post"/><br/><input type="checkbox" name="oh" id="oh" /><label for="oh">'.Language::get('only_headers').'</label><br/><input type="submit" value="'.Language::get('look').'"/></div></form></div>';
        } else {
        ...
        if (isset($_POST['name'], $_POST['host'])) {
            ...
            if (!$factory) {
                ...
                if (isset($_POST['backup'])) {
                    ...
                    echo '<div>&#160;'.$_POST['name'].($_POST['db'] ? ' =&gt; '.\htmlspecialchars($_POST['db'], \ENT_NOQUOTES) : '').'<br/></div>'.$factory->query($_POST['host'], $_POST['name'], $_POST['pass'], $_POST['db'], $_POST['charset'], $_POST['sql']).'<div><form action="?" method="get"><div><input type="hidden" name="gmanager_action" value="change"/><textarea rows="'.(\mb_substr_count($_POST['sql'], "\n") + 1).'" cols="48">'.\htmlspecialchars($_POST['sql'], \ENT_NOQUOTES).'</textarea></div></form></div><div class="input"><form action="?gmanager_action=change&amp;go=mysql&amp;c='.Registry::get('rCurrent').'" method="post" id="post"><div>'.Language::get('sql_query').' <select id="ptn" onchange="Gmanager.paste(this.value);">'.$Patterns->set([Patterns::MySQL])->getOptions().'</select><br/><textarea class="lines" id="sql" name="sql" rows="6" cols="48"></textarea><br/><input type="hidden" name="name" value="'.\htmlspecialchars($_POST['name']).'"/><input type="hidden" name="pass" value="'.\htmlspecialchars($_POST['pass']).'"/><input type="hidden" name="host" value="'.\htmlspecialchars($_POST['host']).'"/><input type="hidden" name="db" value="'.\htmlspecialchars($_POST['db']).'"/><input type="hidden" name="charset" value="'.\htmlspecialchars($_POST['charset']).'"/><input type="submit" value="'.Language::get('sql').'"/>'.($_POST['db'] ? ' <input type="submit" name="backup" value="'.Language::get('sql_backup').'"/>' : '').'</div></form></div>';
                }
            }
        } else {
            ...
            if (!$factory) {
                ...
                if (isset($_POST['backup'])) {
                    if (isset($_POST['file']) && $_POST['file']) {
                        ...
                        $tables = $factory->backup($_POST['host'], $_POST['name'], $_POST['pass'], $_POST['db'], $_POST['charset'], []);
                        echo '<div class="input"><form action="?gmanager_action=change&amp;go=postgresql&amp;c='.Registry::get('rCurrent').'" method="post"><div>'.Language::get('sql_backup_structure').'<br/><select name="tables[]" multiple="true" size="5">'.$tables.'</select><br/>'.Language::get('sql_backup_data').'<br/><select name="data[]" multiple="true" size="5">'.$tables.'</select><br/>'.Language::get('file').'<br/><input type="text" name="file" value="'.Registry::get('hCurrent').'backup_'.\htmlspecialchars($_POST['db']).'.sql"/><br/><input type="hidden" name="name" value="'.\htmlspecialchars($_POST['name']).'"/><input type="hidden" name="pass" value="'.\htmlspecialchars($_POST['pass']).'"/><input type="hidden" name="host" value="'.\htmlspecialchars($_POST['host']).'"/><input type="hidden" name="db" value="'.\htmlspecialchars($_POST['db']).'"/><input type="hidden" name="charset" value="'.\htmlspecialchars($_POST['charset']).'"/><input type="submit" name="backup" value="'.Language::get('sql_backup').'"/></div></form></div>';
                    }
                } else {
                    $Patterns = new Patterns();
                    ...
                    echo '<div>&#160;'.$_POST['name'].($_POST['db'] ? ' =&gt; '.\htmlspecialchars($_POST['db'], \ENT_NOQUOTES) : '').'<br/></div>'.$factory->query($_POST['host'], $_POST['name'], $_POST['pass'], $_POST['db'], $_POST['charset'], $_POST['sql']).'<div><form action="?" method="get"><div><input type="hidden" name="gmanager_action" value="change"/><textarea rows="'.(\mb_substr_count($_POST['sql'], "\n") + 1).'" cols="48">'.\htmlspecialchars($_POST['sql'], \ENT_NOQUOTES).'</textarea></div></form></div><div class="input"><form action="?gmanager_action=change&amp;go=postgresql&amp;c='.Registry::get('rCurrent').'" method="post" id="post"><div>'.Language::get('sql_query').' <select id="ptn" onchange="Gmanager.paste(this.value);">'.$Patterns->set([Patterns::PostgreSQL])->getOptions().'</select><br/><textarea class="lines" id="sql" name="sql" rows="6" cols="48"></textarea><br/><input type="hidden" name="name" value="'.\htmlspecialchars($_POST['name']).'"/><input type="hidden" name="pass" value="'.\htmlspecialchars($_POST['pass']).'"/><input type="hidden" name="host" value="'.\htmlspecialchars($_POST['host']).'"/><input type="hidden" name="db" value="'.\htmlspecialchars($_POST['db']).'"/><input type="hidden" name="charset" value="'.\htmlspecialchars($_POST['charset']).'"/><input type="submit" value="'.Language::get('sql').'"/>'.($_POST['db'] ? ' <input type="submit" name="backup" value="'.Language::get('sql_backup').'"/>' : '').'</div></form></div>';
                }
            }
        } else {
            echo '<div class="input"><form action="?gmanager_action=change&amp;go=postgresql&amp;c='.Registry::get('rCurrent').'" method="post" id="post"><div>'.Language::get('sql_user').'<br/><input type="text" name="name" value=""/><br/>'.Language::get('sql_pass').'<br/><input type="text" name="pass"/><br/>'.Language::get('sql_host').'<br/><input type="text" name="host" value="localhost"/><br/>'.Language::get('sql_db').'<br/><input type="text" name="db"/><br/>'.Language::get('charset').'<br/><input type="text" name="charset" value="utf8"/><br/>'.Language::get('sql_query').'<br/><textarea class="lines" id="sql" name="sql" rows="4" cols="48">'.\htmlspecialchars($_POST['sql'], \ENT_NOQUOTES).'</textarea><br/><input type="submit" value="'.Language::get('sql').'"/></div></form></div>';
    ...
    case 'sqlite':
        ...
        if (isset($_POST['db'])) {
            ...
            if (!$factory) {
                ...
                if (isset($_POST['backup'])) {
                    if (isset($_POST['file']) && $_POST['file']) {
                        echo $factory->backup('', '', '', $_POST['db'], $_POST['charset'], ['tables' => (isset($_POST['tables']) ? \array_map('rawurldecode', $_POST['tables']) : []), 'data' => (isset($_POST['data']) ? \array_map('rawurldecode', $_POST['data']) : []), 'file' => $_POST['file']]);
                    } else {
                        $tables = $factory->backup('', '', '', $_POST['db'], $_POST['charset'], []);
                        echo '<div class="input"><form action="?gmanager_action=change&amp;go=sqlite&amp;c='.Registry::get('rCurrent').'" method="post"><div>'.Language::get('sql_backup_structure').'<br/><select name="tables[]" multiple="true" size="5">'.$tables.'</select><br/>'.Language::get('sql_backup_data').'<br/><select name="data[]" multiple="true" size="5">'.$tables.'</select><br/>'.Language::get('file').'<br/><input type="text" name="file" value="'.Registry::get('hCurrent').'backup_'.\htmlspecialchars(Helper_System::basename($_POST['db'])).'.sql"/><br/><input type="hidden" name="db" value="'.\htmlspecialchars($_POST['db']).'"/><input type="hidden" name="charset" value="'.\htmlspecialchars($_POST['charset']).'"/><input type="submit" name="backup" value="'.Language::get('sql_backup').'"/></div></form></div>';
                    }
                } else {
                    $Patterns = new Patterns();
                    ...
                    echo '<div>&#160;'.$_POST['db'].'<br/></div>'.$factory->query('', '', '', Gmanager::getInstance()->realpath($_POST['db']), $_POST['charset'], $_POST['sql']).'<div><form action="?" method="get"><div><input type="hidden" name="gmanager_action" value="change"/><textarea rows="'.(\mb_substr_count($_POST['sql'], "\n") + 1).'" cols="48">'.\htmlspecialchars($_POST['sql'], \ENT_NOQUOTES).'</textarea></div></form></div><div class="input"><form action="?gmanager_action=change&amp;go=sqlite&amp;c='.Registry::get('rCurrent').'" method="post" id="post"><div>'.Language::get('sql_query').' <select id="ptn" onchange="Gmanager.paste(this.value);">'.$Patterns->set([Patterns::SQLite])->getOptions().'</select><br/><textarea class="lines" id="sql" name="sql" rows="6" cols="48"></textarea><br/><input type="hidden" name="db" value="'.\htmlspecialchars($_POST['db']).'"/><input type="hidden" name="charset" value="'.\htmlspecialchars($_POST['charset']).'"/><input type="submit" value="'.Language::get('sql').'"/> <input type="submit" name="backup" value="'.Language::get('sql_backup').'"/></div></form></div>';
                }
            }
        } else {
    ...
    case 'sql_tables':
        echo '<div class="input">&#187; '.Language::get('select_database').'<br/><div class="red"><ul><li><a href="?gmanager_action=change&amp;go=sql_tables_mysql&amp;c='.Registry::get('rCurrent').'">MySQL</a></li><li><a href="?gmanager_action=change&amp;go=sql_tables_postgresql&amp;c='.Registry::get('rCurrent').'">PostgreSQL</a></li><li><a href="?gmanager_action=change&amp;go=sql_tables_sqlite&amp;c='.Registry::get('rCurrent').'">SQLite</a></li></ul></div></div>';
        break;
    ...
    case 'sql_tables_postgresql':
        if (!(isset($_POST['tables']) && Gmanager::getInstance()->is_file($_POST['tables'])) && !(isset($_FILES['f_tables']) && !$_FILES['f_tables']['error'])) {
            echo '<div class="input"><form action="?gmanager_action=change&amp;go=sql_tables_postgresql&amp;c='.Registry::get('rCurrent').'" method="post" enctype="multipart/form-data"><div>'.Language::get('sql_user').'<br/><input type="text" name="name"/><br/>'.Language::get('sql_pass').'<br/><input type="text" name="pass"/><br/>'.Language::get('sql_host').'<br/><input type="text" name="host" value="localhost"/><br/>'.Language::get('sql_db').'<br/><input type="text" name="db"/><br/>'.Language::get('charset').'<br/><input type="text" name="charset" value="utf8"/><br/>'.Language::get('tables_file').'<br/><input type="text" name="tables" value="'.Registry::get('hCurrent').'" style="width:40%"/><input type="file" name="f_tables" style="width:40%"/><br/><input type="submit" value="'.Language::get('tables').'"/></div></form></div>';
        } else {
            $obj = new SQL();
            $factory = $obj->setDb(SQL::DB_POSTGRESQL)->factory();
            if (!$factory) {
    ...
    case 'sql_installer':
        echo '<div class="input">&#187; '.Language::get('select_database').'<br/><div class="red"><ul><li><a href="?gmanager_action=change&amp;go=sql_installer_mysql&amp;c='.Registry::get('rCurrent').'">MySQL</a></li><li><a href="?gmanager_action=change&amp;go=sql_installer_postgresql&amp;c='.Registry::get('rCurrent').'">PostgreSQL</a></li><li><a href="?gmanager_action=change&amp;go=sql_installer_sqlite&amp;c='.Registry::get('rCurrent').'">SQLite</a></li></ul></div></div>';
        break;
    ...
    case 'sql_installer_postgresql':
        ...
        if (!(isset($_POST['tables']) && Gmanager::getInstance()->is_file($_POST['tables'])) && !(isset($_FILES['f_tables']) && !$_FILES['f_tables']['error'])) {
            echo '<div class="input"><form action="?gmanager_action=change&amp;go=sql_installer_postgresql&amp;c='.Registry::get('rCurrent').'" method="post" enctype="multipart/form-data"><div>'.Language::get('sql_user').'<br/><input type="text" name="name"/><br/>'.Language::get('sql_pass').'<br/><input type="text" name="pass"/><br/>'.Language::get('sql_host').'<br/><input type="text" name="host" value="localhost"/><br/>'.Language::get('sql_db').'<br/><input type="text" name="db"/><br/>'.Language::get('charset').'<br/><input type="text" name="charset" value="utf8"/><br/>'.Language::get('tables_file').'<br/><input type="text" name="tables" value="'.Registry::get('hCurrent').'" style="width:40%"/><input type="file" name="f_tables" style="width:40%"/><br/><input onkeypress="return Gmanager.number(event)" type="text" name="chmod" size="4" maxlength="4" style="-wap-input-format:\'4N\';width:28pt;" value="0644"/>'.Language::get('chmod').'<br/><input name="save_as" type="submit" value="'.Language::get('save_as').'"/><input type="text" name="file" value="'.$d.'sql_installer.php"/><br/></div></form></div>';
        } else {
            $obj = new SQL();
    ...
    case 'sql_installer_sqlite':
        ...
        if (!(isset($_POST['tables']) && Gmanager::getInstance()->is_file($_POST['tables'])) && !(isset($_FILES['f_tables']) && !$_FILES['f_tables']['error'])) {
            echo '<div class="input"><form action="?gmanager_action=change&amp;go=sql_installer_sqlite&amp;c='.Registry::get('rCurrent').'" method="post" enctype="multipart/form-data"><div>'.Language::get('sql_db').'<br/><input type="text" name="db"/><br/>'.Language::get('tables_file').'<br/><input type="text" name="tables" value="'.Registry::get('hCurrent').'" style="width:40%"/><input type="file" name="f_tables" style="width:40%"/><br/><input onkeypress="return Gmanager.number(event)" type="text" name="chmod" size="4" maxlength="4" style="-wap-input-format:\'4N\';width:28pt;" value="0644"/>'.Language::get('chmod').'<br/><input name="save_as" type="submit" value="'.Language::get('save_as').'"/><input type="text" name="file" value="'.$d.'sql_installer.php"/><br/></div></form></div>';
        } else {
            $obj = new SQL();
    ...
    case 'cmd':
        ...
        echo '<div class="input"><form action="?gmanager_action=change&amp;go=cmd&amp;c='.Registry::get('rCurrent').'" method="post"><div>'.Language::get('cmd_code').'<br/><input type="text" name="cmd" value="'.$v.'" style="width:98%"/><br/><input type="submit" value="'.Language::get('cmd_go').'"/></div></form></div>';
        break;
