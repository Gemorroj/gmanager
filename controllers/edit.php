 ...
 */
if (isset($_POST['get'])) {
    \header('Location: '.GMANAGER_URL.'?gmanager_action=change&get='.\rawurlencode($_GET['c'].($_GET['f'] ? '&f='.$_GET['f'] : '')));
    exit;
}
...
$_GET['go'] = $_GET['go'] ?? '';
$_GET['c'] = $_GET['c'] ?? '';
$_GET['charset'] = $_GET['charset'] ?? '';
$_GET['beautify'] = $_GET['beautify'] ?? '';
...
if ($_GET['charset'] && 'default' !== $_GET['charset']) {
    [$charset[0], $charset[1]] = \explode(' -> ', $_GET['charset']);
...
switch (isset($_POST['editorSave']) ? 'save' : $_GET['go']) {
    case 'save':
        if (Registry::get('lineEditor') && isset($_POST['start'], $_POST['end'])) {
    ...
    default:
        if ('file' != Registry::get('currentType') && 'file' != Registry::get('currentTypeLink')) {
        ...
        echo '<div class="input">'.$data['lines'].' '.Language::get('lines').' / '.$data['size'].'<form action="?gmanager_action=edit&amp;c='.Registry::get('rCurrent').$f.'" method="post"><div class="edit">'.$edit.'</div><fieldset class="edit">'.$appendEdit.'<input name="editorSave" type="submit" value="'.Language::get('save').'"/><select name="charset"><option value="utf-8">utf-8</option><option value="windows-1251"'.('windows-1251' == $charset[1] ? ' selected="selected"' : '').'>windows-1251</option><option value="iso-8859-1"'.('iso-8859-1' == $charset[1] ? ' selected="selected"' : '').'>iso-8859-1</option><option value="cp866"'.('cp866' == $charset[1] ? ' selected="selected"' : '').'>cp866</option><option value="koi8-r"'.('koi8-r' == $charset[1] ? ' selected="selected"' : '').'>koi8-r</option></select><br/>'.Language::get('chmod').' <input onkeypress="return Gmanager.number(event)" type="text" name="chmod" value="'.Gmanager::getInstance()->lookChmod(Registry::get('current')).'" size="4" maxlength="4" style="-wap-input-format:\'4N\';width:28pt;"/><br/><input type="submit" name="get" value="'.Language::get('get').'"/><br/>'.($isLineEditor ? '<a href="?gmanager_action=edit&amp;lineEditor=0&amp;c='.Registry::get('rCurrent').$f.'">'.Language::get('basic_editor').'</a>' : Language::get('basic_editor')).' / '.($isLineEditor ? Language::get('line_editor') : '<a href="?gmanager_action=edit&amp;lineEditor=1&amp;c='.Registry::get('rCurrent').$f.'">'.Language::get('line_editor').'</a>').'</fieldset><fieldset class="edit">'.Language::get('replace_from').'<br/><input type="text" name="from" value="'.(isset($_POST['from']) ? \htmlspecialchars($_POST['from']) : '').'" style="width:128pt;"/>'.Language::get('replace_to').'<input type="text" name="to" value="'.(isset($_POST['to']) ? \htmlspecialchars($_POST['to']) : '').'" style="width:128pt;"/><br/><input type="checkbox" name="regexp" id="regexp" value="1"'.(isset($_POST['regexp']) ? ' checked="checked"' : '').'/><label for="regexp">'.Language::get('regexp').'</label><br/><input type="checkbox" name="case" id="case" value="1"'.(isset($_POST['case']) ? ' checked="checked"' : '').'/><label for="case">'.Language::get('register').'</label><br/><input type="submit" name="editorReplace" value="'.Language::get('replace').'"/></fieldset></form></div>'.$http.'<div class="rb"><a href="?gmanager_action=edit&amp;c='.Registry::get('rCurrent').$f.'&amp;'.$full_charset.'go=syntax">'.Language::get('syntax').'</a><br/></div>';

        if ('' == $archive && \extension_loaded('xml')) {
            echo '<div class="rb"><a href="?gmanager_action=edit&amp;c='.Registry::get('rCurrent').'&amp;'.$full_charset.'go=validator">'.Language::get('validator').'</a><br/></div>';
        }

        echo '<div class="rb">'.Language::get('charset').'<form action="?" method="get" style="padding:0;margin:0;"><div><input type="hidden" name="gmanager_action" value="edit"/><input type="hidden" name="c" value="'.Registry::get('hCurrent').'"/><input type="hidden" name="f" value="'.\rawurlencode($_GET['f']).'"/>'.($isLineEditor ? '<input type="hidden" name="start" value="'.($start + 1).'"/><input type="hidden" name="end" value="'.$end.'"/>' : '').'<select name="charset"><option value="default">'.Language::get('charset_no').'</option><optgroup label="UTF-8"><option value="utf-8 -&gt; windows-1251"'.('utf-8 -> windows-1251' == $_GET['charset'] ? ' selected="selected"' : '').'>utf-8 -&gt; windows-1251</option><option value="utf-8 -&gt; iso-8859-1"'.('utf-8 -> iso-8859-1' == $_GET['charset'] ? ' selected="selected"' : '').'>utf-8 -&gt; iso-8859-1</option><option value="utf-8 -&gt; cp866"'.('utf-8 -> cp866' == $_GET['charset'] ? ' selected="selected"' : '').'>utf-8 -&gt; cp866</option><option value="utf-8 -&gt; koi8-r"'.('utf-8 -> koi8-r' == $_GET['charset'] ? ' selected="selected"' : '').'>utf-8 -&gt; koi8-r</option></optgroup><optgroup label="Windows-1251"><option value="windows-1251 -&gt; utf-8"'.('windows-1251 -> utf-8' == $_GET['charset'] ? ' selected="selected"' : '').'>windows-1251 -&gt; utf-8</option><option value="windows-1251 -&gt; iso-8859-1"'.('windows-1251 -> iso-8859-1' == $_GET['charset'] ? ' selected="selected"' : '').'>windows-1251 -&gt; iso-8859-1</option><option value="windows-1251 -&gt; cp866"'.('windows-1251 -> cp866' == $_GET['charset'] ? ' selected="selected"' : '').'>windows-1251 -&gt; cp866</option><option value="windows-1251 -&gt; koi8-r"'.('windows-1251 -> koi8-r' == $_GET['charset'] ? ' selected="selected"' : '').'>windows-1251 -&gt; koi8-r</option></optgroup><optgroup label="ISO-8859-1"><option value="iso-8859-1 -&gt; utf-8"'.('iso-8859-1 -> utf-8' == $_GET['charset'] ? ' selected="selected"' : '').'>iso-8859-1 -&gt; utf-8</option><option value="iso-8859-1 -&gt; windows-1251"'.('iso-8859-1 -> windows-1251' == $_GET['charset'] ? ' selected="selected"' : '').'>iso-8859-1 -&gt; windows-1251</option><option value="iso-8859-1 -&gt; cp866"'.('iso-8859-1 -> cp866' == $_GET['charset'] ? ' selected="selected"' : '').'>iso-8859-1 -&gt; cp866</option><option value="iso-8859-1 -&gt; koi8-r"'.('iso-8859-1 -> koi8-r' == $_GET['charset'] ? ' selected="selected"' : '').'>iso-8859-1 -&gt; koi8-r</option></optgroup><optgroup label="CP866"><option value="cp866 -&gt; utf-8"'.('cp866 -> utf-8' == $_GET['charset'] ? ' selected="selected"' : '').'>cp866 -&gt; utf-8</option><option value="cp866 -&gt; windows-1251"'.('cp866 -> windows-1251' == $_GET['charset'] ? ' selected="selected"' : '').'>cp866 -&gt; windows-1251</option><option value="cp866 -&gt; iso-8859-1"'.('cp866 -> iso-8859-1' == $_GET['charset'] ? ' selected="selected"' : '').'>cp866 -&gt; iso-8859-1</option><option value="cp866 -&gt; koi8-r"'.('cp866 -> koi8-r' == $_GET['charset'] ? ' selected="selected"' : '').'>cp866 -&gt; koi8-r</option></optgroup><optgroup label="KOI8-R"><option value="koi8-r -&gt; utf-8"'.('koi8-r -> utf-8' == $_GET['charset'] ? ' selected="selected"' : '').'>koi8-r -&gt; utf-8</option><option value="koi8-r -&gt; windows-1251"'.('koi8-r -> windows-1251' == $_GET['charset'] ? ' selected="selected"' : '').'>koi8-r -&gt; windows-1251</option><option value="koi8-r -&gt; iso-8859-1"'.('koi8-r -> iso-8859-1' == $_GET['charset'] ? ' selected="selected"' : '').'>koi8-r -&gt; iso-8859-1</option><option value="koi8-r -&gt; cp866"'.('koi8-r -> cp866' == $_GET['charset'] ? ' selected="selected"' : '').'>koi8-r -&gt; cp866</option></optgroup></select> <input type="submit" value="'.Language::get('ch').'"/></div></form></div><div class="rb">'.Language::get('beautifier').' (alpha)<form action="?" method="get" style="padding:0;margin:0;"><div><input type="hidden" name="gmanager_action" value="edit"/><input type="hidden" name="beautify" value="1"/><input type="hidden" name="c" value="'.Registry::get('hCurrent').'"/><input type="hidden" name="f" value="'.\rawurlencode($_GET['f']).'"/>'.($isLineEditor ? '<input type="hidden" name="start" value="'.($start + 1).'"/><input type="hidden" name="end" value="'.$end.'"/>' : '').'<input type="submit" value="'.Language::get('beautify').'" /></div></form></div>';
        break;
}
